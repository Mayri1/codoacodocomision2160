...
public class AlumnosDAO {
    ...
    public boolean actualizarAlumno(Alumnos alumno){
        PreparedStatement ps;
        try{
             ps = conexion.prepareStatement("UPDATE participantess SET nombres=?, apellidos=?, email=?, telefono=? where id=?");
             ps.setString(1,alumno.getNombres());
             ps.setString(2,alumno.getApellidos());
             ps.setString(3,alumno.getEmail());
             ps.setInt(4, alumno.getTelefono());
    ...
    public boolean eliminarAlumno(int _id){
        PreparedStatement ps;
          try{
            ps = conexion.prepareStatement("DELETE FROM participantess WHERE id=?");
            ps.setInt(1,_id);
            ps.execute();
            return true;
          }catch(SQLException e){
